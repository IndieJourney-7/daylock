/**
 * Admin → User Personal Analytics
 * Full personal progress dashboard for a specific user
 * Same richness as the user's own analytics — streaks, heatmap, charts, export
 */
...
function Skeleton() {
  return (
    <div className="max-w-5xl mx-auto space-y-6 animate-pulse">
      <div className="h-8 w-60 bg-charcoal-600 rounded" />
...
const DONUT_COLORS = ['#22c55e', '#ef4444', '#f59e0b', '#6366f1', '#3b82f6', '#ec4899']
  ...
  if (error || !data) {
    return (
      <div className="max-w-5xl mx-auto text-center py-12">
        ...
        <h3 className="text-white font-semibold mb-1">Failed to load user analytics</h3>
        <p className="text-gray-500 text-sm mb-4">{error || 'User not found or you don\'t manage any rooms for this user.'}</p>
  ...
  if (data.overview?.totalDays === 0) {
    return (
      <div className="max-w-5xl mx-auto text-center py-16">
        ...
        <h3 className="text-white font-semibold text-lg mb-2">No attendance data yet</h3>
        <p className="text-gray-500 text-sm mb-4">
          {data.user?.name || 'This user'} hasn't submitted any attendance in your rooms yet.
        </p>
        <Button variant="secondary" onClick={() => navigate('/admin/analytics')}>
          Back to Analytics
        </Button>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full bg-charcoal-500/50 flex items-center justify-center">
              {data.user?.avatar_url ? (
                <img src={data.user.avatar_url} alt="" className="w-12 h-12 rounded-full object-cover" />
              ) : (
                <span className="text-accent text-lg font-bold">{userName[0]?.toUpperCase()}</span>
              )}
            </div>
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-white">{userName}</h1>
              <p className="text-gray-500 text-sm">
                {data.totalRooms} room{data.totalRooms !== 1 ? 's' : ''} managed by you
              </p>
            </div>
          </div>
        </div>
      ...
      <Card className="p-6">
        ...
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={weeklyTrend}>
            <defs>
              <linearGradient id="userRateGrad" x1="0" y1="0" x2="0" y2="1">
            ...
            <Area
              ...
              stroke="#8b5cf6" fill="url(#userRateGrad)" strokeWidth={2}
            />
          </AreaChart>
        </ResponsiveContainer>
      </Card>
            ...
            {roomBreakdown.map((room, i) => (
              <Card key={room.roomId} className="p-4">
                ...
                <div className="flex items-center gap-3 text-xs">
                  <span className="text-green-400">{room.approved} approved</span>
                  <span className="text-red-400">{room.rejected} rejected</span>
                  <span className="text-yellow-400">{room.missed} missed</span>
                </div>
              </Card>
            ))}
          </div>
                  ...
                  return (
                    <tr key={rec.id} className="border-b border-charcoal-400/5 hover:bg-charcoal-500/10">
                      ...
                      <td className="py-2.5 pr-4 text-gray-300">
                        <span className="mr-1">{rec.room?.emoji}</span>
                        {rec.room?.name || '-'}
                      </td>
